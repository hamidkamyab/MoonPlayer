...
const FavlistSection = (props) => {

    const renderItem = ({ item, index }) => (
        <Pressable onPress={() => props.playSelectTrack(index+1)} >
    ...
    return (
        <>
            <Modal isOpen={props.isOpenFavlist} onClose={props.isCloseFavlist}>
                <VStack style={styles.ModalContent} space={4} >
                    <HStack alignItems={'center'} space={2} pb={2} justifyContent={'space-between'} borderBottomColor={'#666'} borderBottomWidth={1} >
                        <Box>
                            <Text fontSize={20} color={'#666'}>Favorite List</Text>
                        </Box>
                        <Pressable onPress={props.isCloseFavlist}>
                            <Icon as={EvilIcons} name='close' size={25} color={'#666'} />
                        </Pressable>
                    </HStack>

                    <FlatList
                        data={props.favoriteSongs}
